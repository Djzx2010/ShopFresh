<?php
if($_POST){
// Store the information of the form in variable
$name = $_POST["name"];
$quantity = $_POST["quantity"];
$type = $_POST["type"];
$size = $_POST["size"];
// Create the new xml element
$xml = new SimpleXMLElement("product.xml",0,true);
$newProduct = $xml->addChild("product");
$newProduct->addChild("name", $name);
$newProduct->addChild("quantity", $quantity);
$newProduct->addChild("type", $type);
$newProduct->addChild("size",$size);
//Add the product to the xml file
$xml->asXML("product.xml");

//---------------------------------------------------------------------------------------------------------------------------------------
// CREATE THE EDIT PAGE FOR THE PRODUCT
//---------------------------------------------------------------------------------------------------------------------------------------
//Edit page information stored in a variable
$editPageInfo = "
<?php
if(\$_POST){
\$quantity = \$_POST[\"quantity\"];
...
<html lang=\"eng\" dir=\"ltr\">
   ...
   <body>
    ...
    <nav class=\"navbar navbar-expand-lg navbar-light\" style=\"background-color:#95d097\">
      ...
      <div class=\"editForm\">
       <h3>{$name}</h3>  
       <form action=\" {$name}(P8).php \" method=\"post\">
...
";
//Creating the file and writing to the file
